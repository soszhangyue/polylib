...
def lin_as_mat(start:int, end:int, gap = 1, int_type = False):
    ...
    if end < start:
        ...
        if int_type:
            return (np.linspace(start, start + gap * (array_length - 1) ,num = array_length, endpoint= True,dtype=int)).reshape([1, array_length])
        else:
            return (np.linspace(start, start + gap * (array_length - 1) ,num = array_length, endpoint= True)).reshape([1, array_length])
